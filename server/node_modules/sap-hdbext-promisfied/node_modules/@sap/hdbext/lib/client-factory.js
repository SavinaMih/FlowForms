...
module.exports.isHanaCloud = isHanaCloud;

async function isHanaCloud(client) {
  try {
    const result = await hdbPromise.exec(client, 'SELECT VERSION FROM "SYS"."M_DATABASE"');
    ...
    debug('Error determining HANA Cloud:', err);
  }
  return false;
}
...
function createConnection(options, callback) {
  ...
  async.series([
    ...
    function (cb) {
      ...
      isHanaCloud(client)
        .then((hanaCloud) => {
          const variableName = hanaCloud ? 'APPLICATIONUSER' : 'XS_APPLICATIONUSER';
          client.exec(`SET '${variableName}' = ${safeSql.stringLiteral(xsAppUser)}`, cb);
        })
        .catch((err) => {
          debug('Error determining and setting HANA Cloud context:', err);
          cb(err);
        });
    }
  ], function (err) {
...
async function createConnectionPromise(options) {
  ...
  try {
    var xsAppUser = options['sessionVariable:XS_APPLICATIONUSER'];
    if (xsAppUser) {
      const hanaCloud = await isHanaCloud(conn);
      const variableName = hanaCloud ? 'APPLICATIONUSER' : 'XS_APPLICATIONUSER';
      await hdbPromise.exec(conn, `SET '${variableName}' = ${safeSql.stringLiteral(xsAppUser)}`);
    }
    return conn;
  }
  catch (err) {
    try {
      ...
      debug('Error while closing connection.', error);
    }
    throw (err);
  }
}
